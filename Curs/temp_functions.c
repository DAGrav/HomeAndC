...
void PrintHelp (bool mode, const char* fileName)
{
    ...
    printf("-d Quantity\t Display the number of the damaged line. The quantity must be from 0 to 255\n");
}
...
void PrintDamageNums (false_data* Items, uint8_t countPrint)
{
    printf("Damage lines:\n");
    printf("\t=============================\n");
    unsigned int index = 0, count = countPrint;
    while (index < Items->count)
    {
        if (index == count && countPrint > 0)
        {
            printf("\t=============================\n");
            puts("To exit, type 'q', to continue output, press Enter...");
            int answer;
            while ((answer != '\n' && answer != 'q') && (answer != '\n' && answer != 'Q'))
            {
                answer = getchar();
...
void GetYearStat (stat_t* yearItem, months_stat* monthItems)
{
    ...
    if (monthItems->count == 0)
        return;
    unsigned int averCount = 0;
    yearItem->maxTemp = INT8MIN;
    yearItem->minTemp = INT8MAX;
    yearItem->averTemp = 0.0;
    for (unsigned int index = 0; index < monthItems->count; index++)
    {
...
uint16_t GetMonthStat (months_stat* items, sensor_data* sensorItems)
{
    ...
    mntItem.yrOrMnth = 0;
    for (unsigned int index = 0; index < sensorItems->count; index++)
    {
        if (index == 0)
