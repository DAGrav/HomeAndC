...
void refreshFood(struct food f[], int nfood)
{
    ...
    for(size_t i=0; i<nfood; i++)
    {
        if( f[i].put_time )
        {
            if( !f[i].enable || (time(NULL) - f[i].put_time) >
                    FOOD_EXPIRE_SECONDS )
            {
                putFoodSeed(&f[i]);
            }
...
{
    for (int i = 0; i < CONTROLS; i++)
        // Блоки проверки движения в хвост
...
int tail_collision(struct snake_t *head, const int key_code)
{
    for(size_t counter = head->tsize-1; counter > 0; counter--)
        if (head->x == head->tail[counter].x &&
                head->y == head->tail[counter].y) {
            return STOP_GAME; // столкновение с хвостом
        }
    return key_code;
}
void ShowKeyPress (const int key_code)
{
     switch(key_code) {
            case KEY_UP:
                mvprintw(1, 1,"  KEY_UP   ");
                break;
            case KEY_DOWN:
                mvprintw(1, 1," KEY_DOWN ");
                break;
            case KEY_LEFT:
                mvprintw(1, 1," KEY_LEFT ");
                break;
            case KEY_RIGHT:
                mvprintw(1, 1," KEY_RIGHT ");
                break;
            default:
                if (key_code >= 32 && key_code <= 126) { // Печатные символы
                    mvprintw(1, 1,"       '%c'     ", key_code);
                }// else {
                    //mvprintw(0, 0,"Специальная клавиша (код: %d)", key_code);
               // }
        }
    }
...
int main()
{
    ...
    while( key_pressed != STOP_GAME )
    {
        ...
        goTail(snake);
        clock_t begin = clock();
        double time_w = (double)100 / 1000.0;
        while (((double)(clock() - begin) / CLOCKS_PER_SEC) < time_w) {
        }
        if (checkDirection(snake, key_pressed ))
            changeDirection(snake, key_pressed);
        key_pressed = tail_collision (snake, key_pressed); //проверка на столкновение с хвостом
